...
lexeme default = action => [ name, values ] latm => 1
    ...
    rule ::= <symbol list> '::=' op
    op   ::= <op swap> | <op set> | <op insert> | <op remove>
    <op swap>   ::= <symbol list> '~=' <node list>  # swap -- rearrange children # higher priority op
    <op set>    ::= <symbol list>  '=' <node list>  # set # higher priority op
    <op insert> ::= <index list>  '+=' <node list>  # insert
    <op remove> ::= <index list>  '-=' <node list>  # remove if nodes in <node list> exist at indices as <index list>
    <op remove> ::= <index list>  '-='              # remove without cheking

    # sink rule, to be removed
    op ::= expr
